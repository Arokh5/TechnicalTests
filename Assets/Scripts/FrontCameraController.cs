...
public class FrontCameraController : MonoBehaviour
{
    ...
    public Vector3 pivotOffset = new Vector3(0.0f, 2.0f, 0.0f);        // Offset to repoint the camera.
    public Vector3 camOffset = new Vector3(0f, 0f, -7.0f);             // Offset to relocate the camera related to the player position.
    ...
    void Awake()
    {
        ...
        cam.rotation = Quaternion.Euler(0f, 180f, 0f);
    ...
    // Set camera offset.
    public void SetCamOffset(ref Vector3 camOffset)
    {
        if (camOffset.z < -15f)
        {
            camOffset.z = -15f;
    ...
    bool ReverseViewingPosCheck(Vector3 checkPos, float deltaPlayerHeight, float maxDistance)
    {
        ...
        {
            if (hit.transform != transform && hit.transform.gameObject.layer == environmentLayer)
            {
                return false;
            }
        }
